...
export default function ErfahrungsraumFormatePage() {
  const navigate = useNavigate();
  const [records, setRecords] = useState<ErfahrungsraumFormate[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<ErfahrungsraumFormate | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<ErfahrungsraumFormate | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getErfahrungsraumFormate());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: ErfahrungsraumFormate['fields']) {
    await LivingAppsService.createErfahrungsraumFormateEntry(fields);
    await loadData();
    setDialogOpen(false);
  }
  ...
  async function handleUpdate(fields: ErfahrungsraumFormate['fields']) {
    ...
    await LivingAppsService.updateErfahrungsraumFormateEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteErfahrungsraumFormateEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Erfahrungsraum-Formate"
      subtitle={`${records.length} Erfahrungsraum-Formate im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Erfahrungsraum-Formate suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('zielsetzung')}>
                <span className="inline-flex items-center gap-1">
                  Zielsetzung
                  {sortKey === 'zielsetzung' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('methodik')}>
                <span className="inline-flex items-center gap-1">
                  Methodik
                  {sortKey === 'methodik' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('weitere_methoden')}>
                <span className="inline-flex items-center gap-1">
                  Weitere Methoden / Ergänzungen
                  {sortKey === 'weitere_methoden' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/erfahrungsraum-formate/${record.record_id}`); }}>
                ...
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.zielsetzung ?? '—'}</span></TableCell>
                <TableCell>{record.fields.zielgruppe ?? '—'}</TableCell>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{record.fields.methodik?.label ?? '—'}</span></TableCell>
                <TableCell>{record.fields.weitere_methoden ?? '—'}</TableCell>
                ...
                <TableCell>{record.fields.materialien ? <div className="relative h-8 w-8 rounded bg-muted overflow-hidden"><div className="absolute inset-0 flex items-center justify-center"><IconFileText size={14} className="text-muted-foreground" /></div><img src={record.fields.materialien} alt="" className="relative h-full w-full object-cover" onError={e => { (e.target as HTMLImageElement).style.display = "none"; }} /></div> : '—'}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={12} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Erfahrungsraum-Formate. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
